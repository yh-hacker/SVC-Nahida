...
def padDiff(x):
    x = x.unsqueeze(0) # 为了能用paddle的pad函数
...
class Generator(paddle.nn.Layer):
    def __init__(self, h):
        ...
        self.f0_upsamp = paddle.nn.Upsample(scale_factor=paddle.prod(paddle.to_tensor(h["upsample_rates"]),dtype = 'float32'),data_format = 'NCW',mode = 'linear')
    ...
    def forward(self, x, f0, g=None):
        ...
        vt = f0[:, None]#.astype('float64')
        f0 = self.f0_upsamp(vt)
